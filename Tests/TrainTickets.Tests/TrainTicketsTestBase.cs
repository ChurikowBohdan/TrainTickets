...
using TrainTickets.Migrations.SeedData;
...
{
    public abstract class TrainTicketsTestBase : AbpIntegratedTestBase<TrainTicketsTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TrainTicketsTestBase()
        {
        ...
        protected void UsingDbContext(Action<TrainTicketsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TrainTicketsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TrainTicketsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TrainTicketsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TrainTicketsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TrainTicketsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TrainTicketsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TrainTicketsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TrainTicketsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TrainTicketsDbContext, Task<T>> func)
        {
            T result;
